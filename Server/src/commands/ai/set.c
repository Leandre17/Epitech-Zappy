...
void send_set_output_to_all_graphics(
    server_t *server,
    client_t *client,
    int pos[2],
    char *resource
)
{
    ...
    send_pdr_to_all_graphics(server, client->player, resource_id);
...
)
{
    ...
    if (!server || !server->map || !server->map->tiles ||
        ...
        exit_error("command_set()");
    if (arrlen(args) != 2)
        return send_to_user(client, API_NOT_FOUND);
    pos[0] = client->player->pos_x;
    pos[1] = client->player->pos_y;
    if (!set_resource(server->map->tiles, client->player->inventory,
        pos, args[1]))
        return send_to_user(client, API_NOT_FOUND);
    send_set_output_to_all_graphics(server, client, pos, args[1]);
